	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response.ok) {
				...
				router.push(
					"/auth/login?message=Account created successfully! Please log in with your email.",
				);
			} else {
		...
		<section className="relative w-full min-h-screen bg-cover bg-center bg-no-repeat flex items-end">
			...
			<div
				...
				<p className="mt-2 text-gray-600">
					Join AulonaFlows and start your wellness journey
				</p>
			</div>
			...
			<div className="grid place-items-center h-screen w-full relative z-50">
				<Card className="bg-transparent/50 backdrop-blur-sm border border-white/15 py-8 px-7 rounded-2xl h-auto w-[90%] md:w-[30%]">
					<CardHeader>
						...
						/>
						<CardTitle className="text-white text-center text-2xl font-medium">
							Create Your Account
						</CardTitle>
						<p className="text-white/70 text-sm text-center">
							Sign Up to get easy access to all your bookings and exclusive
							Aulona Flows deals.
						</p>
					</CardHeader>
					<CardContent>
						...
						<form
							...
							<div>
								<Label
									htmlFor="firstName"
									className="text-white mb-1"></Label>
								<Input
									id="firstName"
